...
class Hiwi:
	def __init__(self, settings_file="settings.py"):
		...
		self.repository_url = "http://localhost:5000"
	...
	def integrate_from_web(self, package_name):
		#print(self.repository_url + "/get/" + package_name)
		r = requests.get(self.repository_url + "/get/" + package_name)
		if r.status_code != 200:
			return False

		with open('tmpzip.zip', 'wb') as f:
			f.write(r.content)
		
		self.integrate('tmpzip')
		os.remove("tmpzip.zip")


	def publish(self, name):
		zip_filename = self.package(name)
		with open(zip_filename, 'rb') as f:
			r = requests.post(self.repository_url + "/put", files={'package': f})
